    ...
    <div class="mx-auto max-w-7xl space-y-8">
      <header class="flex items-end justify-between">
        <div>
          ...
          <p class="mt-1 text-sm text-muted">
            Track project budgets, expenses, and studio burn rate.
          </p>
        </div>
      </header>
      ...
      <section class="grid grid-cols-1 gap-6 md:grid-cols-3">
        <div class="project-card border-green/20 bg-green/5">
          <p class="text-[10px] font-bold uppercase tracking-widest text-green">
            Total Revenue
          </p>
          <p class="mt-4 font-display text-3xl font-bold text-green">
            ${{ totalRevenue.toLocaleString() }}
          </p>
        </div>
        <div class="project-card border-red/20 bg-red/5">
          <p class="text-[10px] font-bold uppercase tracking-widest text-red">
            Total Expenses
          </p>
          <p class="mt-4 font-display text-3xl font-bold text-red">
            ${{ totalExpenses.toLocaleString() }}
          </p>
        </div>
        <div class="project-card border-brand/20 bg-brand/5">
          <p class="text-[10px] font-bold uppercase tracking-widest text-brand">
            Net Profit
          </p>
          <p class="mt-4 font-display text-3xl font-bold text-text">
      ...
      <section class="dashboard-panel">
        <h3 class="text-xs font-bold text-muted uppercase tracking-widest mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <button
            ...
            :disabled="!newEntry.project || !newEntry.amount || newEntry.amount <= 0"
        ...
        <p
          ...
          class="mt-3 text-xs font-semibold text-red"
        >
          {{ validationError }}
        </p>
      </section>
      ...
      <div
        v-if="saved"
        class="fixed bottom-4 right-4 bg-green/10 border border-green/30 text-green p-4 rounded-xl shadow-2xl z-50 flex items-center gap-3 min-w-[250px]"
      >
        ...
        <table class="w-full text-left text-sm">
          ...
          <tbody class="divide-y divide-border/50">
            <tr
              ...
              :key="idx"
              class="bg-surface/50 hover:bg-panel/30 transition-colors"
            >
              ...
              <td class="px-6 py-4">
                <span
                  class="badge"
                  :class="tx.type === 'Income' ? 'badge-green' : 'badge-brand'"
                >{{ tx.type }}</span>
              </td>
              <td class="px-6 py-4 text-muted">
                {{ tx.desc }}
              ...
              <td
                ...
                :class="tx.type === 'Income' ? 'text-green' : 'text-red'"
              >
