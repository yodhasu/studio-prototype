    ...
    >
      ...
      <div class="p-4">
        <div class="flex items-start justify-between gap-3 mb-4">
          ...
          <h3
            v-else
            class="font-display text-sm font-bold text-text truncate leading-tight"
          >
            {{ card.title }}
          </h3>
          <component
            :is="cardIcon"
            class="h-4 w-4 shrink-0 text-brand/60"
          />
        </div>

        <p
          v-if="card.description"
          class="mb-4 text-[11px] text-muted line-clamp-2 italic leading-relaxed"
        >
          {{ card.description }}
        </p>
        ...
        <div class="space-y-1.5">
          ...
          <div class="sketch-border bg-[var(--c-bg)]/20 overflow-hidden border-border/60">
            ...
            >
              ...
              <component
                :is="collapsed.notes ? ChevronRight : ChevronDown"
                class="h-2.5 w-2.5 opacity-50"
              />
            </button>
            <div
              v-show="!collapsed.notes"
            ...
            >
              ...
              <div
                v-else
                class="text-xs text-muted leading-relaxed line-clamp-4 whitespace-pre-wrap"
              >
          ...
          <div class="sketch-border bg-[var(--c-bg)]/20 overflow-hidden border-border/60">
            ...
            >
              ...
              <component
                :is="collapsed.media ? ChevronRight : ChevronDown"
                class="h-2.5 w-2.5 opacity-50"
              />
            </button>
            <div
              v-show="!collapsed.media"
              class="px-2 pb-3 pt-1"
            >
              <!-- Media Grid (Max 9) -->
              <div
                v-if="card.content?.media?.length"
              ...
              >
                ...
                >
                  <component
                    ...
                    class="w-4 h-4 text-faint group-hover/media:text-brand transition-colors"
                  />
                  <div class="absolute inset-0 bg-brand/5 opacity-0 group-hover/media:opacity-100 transition-opacity" />
                </div>
              ...
              <div
                v-if="collapsed.media && card.content?.media?.length"
                class="flex -space-x-1 mb-1 mt-2"
              >
                <div
                  v-for="(item, idx) in card.content.media.slice(0, 4)"
                  :key="idx"
                  class="h-5 w-5 rounded border border-surface bg-panel/60 p-1"
                >
                  <component
                    :is="getMediaIcon(item.type)"
                    class="w-full h-full text-brand/40"
                  />
                </div>
                <div
                  v-if="card.content.media.length > 4"
                  class="h-5 w-5 rounded border border-surface bg-panel/60 flex items-center justify-center text-[8px] font-bold text-faint"
                >
        ...
        <div
          v-if="!isSelected && card.content?.media?.length"
        ...
        <div
          v-if="isSelected"
          class="sketch-border bg-[var(--c-bg)]/20 overflow-hidden border-border/60 opacity-40"
        >
          <button
            class="w-full flex items-center justify-between px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-muted hover:bg-[var(--c-bg)]/30 transition-colors"
          ...
          >
            ...
            <component
              :is="collapsed.tasks ? ChevronRight : ChevronDown"
              class="h-2.5 w-2.5 opacity-50"
            />
          </button>
          <div
            v-show="!collapsed.tasks"
            class="px-3 pb-3 pt-1"
          >
...
type CardUpdatePayload = { id: Card['id'] } & Partial<Omit<Card, 'id' | 'workspace_id' | 'org_id'>>
...
const emit = defineEmits<{
  ...
  'update': [payload: CardUpdatePayload]
